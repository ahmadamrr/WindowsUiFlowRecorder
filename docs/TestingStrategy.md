...
| **Window-identity deduplication** | Activating/re-activating the *same* harness window N times (including switching away and back per `UC-09`) during one session produces **exactly one** `WindowInformation` entry for it, not N — the window-identity rule in `SystemDesign.md` §9. | Harness window activated/deactivated repeatedly via scripted focus changes. |
| **Click-vs-drag classification accuracy** | A scripted sequence of ordinary clicks with small, realistic sub-pixel/sub-threshold mouse jitter between down and up is classified entirely as `Click`/`RightClick`/`DoubleClick` — zero `Drag` misclassifications — while a displacement deliberately exceeding the OS drag threshold is classified as `Drag`. | Harness's known, fixed-position controls; synthetic input with controlled displacement. |
| **Text-entry keystroke fidelity** | A scripted input string (e.g., a fixed test credential string) typed into a harness text field produces an `EnteredText` value that is character-for-character identical to the scripted input — no duplicated or dropped characters. | Harness's editable text control. |
| **Leaf-element resolution** | Clicking a harness control nested inside a container/composite control (mirroring the ComboBox-wrapping-an-Edit pattern seen in real target applications) resolves `TargetElement` to the specific leaf control actually under the cursor, not the containing control or the window root. | Harness control tree including at least one intentionally nested composite control. |
| **Interaction marking & `HierarchyExportScope` pruning** | After a scripted session touching a known subset of harness controls, the exported `ElementInformation` tree marks exactly that subset `WasInteractedWith = true` with correct `InteractionCount`/`InteractedActionIds`; switching `HierarchyExportScope` across its three values changes the exported node count as specified in `SystemDesign.md` §17.4, while `Actions` remains unchanged in every mode. | Harness's "generate N controls" mode combined with a scripted subset of interactions. |
...
3a. **Capture-quality regression suite.** The window-identity deduplication, click-vs-drag classification, text-entry keystroke fidelity, leaf-element resolution, and interaction-marking/`HierarchyExportScope` tests added to §4 are run on every build, not only periodically — these were identified from a real exported session containing exactly these defects (9 duplicate entries for one physical window, actions misclassified as `Drag`, and duplicated characters in captured text), so they are treated as permanently guarded regressions rather than one-time fixes.
